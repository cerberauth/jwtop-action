...
describe('main.ts', () => {
  beforeEach(() => {
    ...
    fsModule.readFile.mockResolvedValue(
      JSON.stringify({
        metadata: { total: 1, by_severity: { critical: 1 } },
        findings: [
          {
            title: 'Blank Secret',
            severity: 'critical',
            url: 'https://www.cerberauth.com/docs/jwtop/blank-secret/'
          }
        ]
      })
    )
  ...
  describe('PR comment', () => {
    ...
    it('asks jwtop for a full JSON report file and comments with a summary of it', async () => {
      core.getInput.mockImplementation((name: string) => {
        if (name === 'command') return 'crack'
      ...
      expect(execModule.exec).toHaveBeenCalledWith(
        ...
        [
          ...
          '--output',
          expect.stringContaining('reportx-report-'),
          '--output-format',
          'json'
        ],
        expect.objectContaining({ listeners: expect.any(Object) })
      ...
      expect(createComment).toHaveBeenCalledWith(
        expect.objectContaining({
          issue_number: 9,
          body: expect.stringContaining('Blank Secret')
        })
      )
    })

    it('reuses an explicit JSON --output for the comment, and keeps it', async () => {
      core.getInput.mockImplementation((name: string) => {
        if (name === 'command') return 'crack'
        if (name === 'args')
      ...
      expect(fsModule.readFile).toHaveBeenCalledWith('./report.json', 'utf8')
    ...
    it('skips the comment when --output is a non-JSON format, keeping report-path', async () => {
      core.getInput.mockImplementation((name: string) => {
        if (name === 'command') return 'crack'
        if (name === 'args')
    ...
    it('exposes report-path when output-format is set', async () => {
      ...
      // sarif isn't JSON, so there's no structured report to comment with.
      expect(fsModule.readFile).not.toHaveBeenCalled()
      expect(createComment).not.toHaveBeenCalled()
    })
